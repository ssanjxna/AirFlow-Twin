...
async function initAuditLivePage() {
    try {
        const response = await fetch('/api/audit_feed?limit=100');
    ...
    entries.forEach((entry, index) => {
        ...
        card.innerHTML = `
            <div class="flex items-start justify-between gap-3">
                <div>
                    <p class="text-sm font-bold text-white">${entry.flight_id}</p>
            ...
            <div class="flex items-center justify-between mt-3 text-xs text-slate-400">
                ...
                <span>Confidence: ${entry.before_confidence_percent}% -> ${entry.after_confidence_percent}%</span>
            </div>
        `;
...
function renderAuditLiveDetail(entry) {
    ...
    document.getElementById('audit-detail-flight').textContent = entry.flight_id;
    ...
    document.getElementById('audit-detail-confidence-before').textContent = `${entry.before_confidence_percent}%`;
    document.getElementById('audit-detail-confidence-after').textContent = `${entry.after_confidence_percent}%`;
