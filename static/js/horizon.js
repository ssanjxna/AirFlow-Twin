...
async function initHorizonPage() {
    await renderHorizonTimeline();
    setupTimeControls();
}

async function renderHorizonTimeline() {
    ...
    try {
        const response = await fetch('/api/predict/events', {
            ...
            body: JSON.stringify({ time_horizon: currentTimeStep })
        });
        ...
        console.error('Error loading horizon events:', error);
        timeline.innerHTML = '<div class="text-sm text-slate-400">Unable to load live predictions right now.</div>';
    }
}
